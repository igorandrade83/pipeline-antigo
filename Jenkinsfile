pipeline {
    ...
    parameters{
        ...
        string(defaultValue: 'tutorialcronapp/pipeline-antigo', description: 'Informe o nome de sua imagem (se for registry privado, informe o caminho completo).', name: 'CRONAPP_DOCKER_IMAGE_NAME', trim: false)
        string(defaultValue: 'Dockerhub', description: 'Informe a credencial (secret) usada para acesso ao registry.', name: 'CRONAPP_DOCKERHUB_ACCESS', trim: false)
        string(defaultValue: 'https://github.com/igorandrade83/pipeline-antigo.git', description: 'Informe o endereço HTTPS do repositório Git.', name: 'CRONAPP_GIT_URL', trim: false)
        string(defaultValue: 'Github', description: 'Informe a credencial (secret) usada para acesso ao reposotório Git.', name: 'CRONAPP_GIT_USERPASS', trim: false)
    }
    stages {
        stage('Git Clone') {
            steps {
